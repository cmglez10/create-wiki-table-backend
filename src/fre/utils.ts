...
import { RecordResult } from '../interfaces/results.interface';
...
export class FreUtils {
  ...
  static getCompleteName(team$: cheerio.Root): string {
    const cName = FreUtils.addQuotes(split(team$('#derecha_sup_equ').text(), ' :: ')[1]);
  ...
  static normalizeName(name: string): string {
    return FreUtils.addQuotes(replace(trim(replace(name, new RegExp('\\.', 'g'), '. ')), '  ', ' '));
  }
  ...
  static async getRegion(team$: cheerio.Root): Promise<string> {
    ...
    if (links.length === 2) {
      ...
      const provinceHtml = await FreUtils.getHtml(`${FUTBOL_REGIONAL_BASE_URL}${provinceUrl}`);
      const province$ = Cheerio.load(provinceHtml);
      flag = split(split(province$('#derecha_sup_fic').text(), ' :: ')[1], '(')[1]
        .trim()
        .slice(0, -1);
    }
  ...
  static async getTeamInfo(
    ...
    try {
      ...
      const teamInfo: TeamInfo = {
        ...
        coordinates: options.coordinates ? await FreUtils.getCoordinates(team$) : null,
      };
  ...
  static async getCoordinates(team$: cheerio.Root): Promise<Array<string>> {
    ...
    const groundHtml = await FreUtils.getHtml(`${FUTBOL_REGIONAL_BASE_URL}${groundUrl}`);
